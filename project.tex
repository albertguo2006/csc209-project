...
\textbf{Server:} (1)~Game already in progress $\Rightarrow$ \texttt{MSG\_ERROR} ``Game already in progress.''\ + close socket (server.c:251--254). (2)~Lobby full $\Rightarrow$ \texttt{MSG\_ERROR} ``Lobby is full.''\ + close socket (server.c:264--267). (3)~Non-printable bytes in \texttt{name} sanitised to \texttt{'?'} (server.c:260--261); no error is sent. \textbf{Client send:} \texttt{write\_all} failure $\Rightarrow$ \texttt{perror("write")} + \texttt{exit(1)} (client.c:505--508). \\[4pt]
...
\textbf{Server:} (1)~Wrong game phase $\Rightarrow$ \texttt{MSG\_ERROR} ``Not currently accepting actions.''\ (server.c:318--320). (2)~Duplicate submission $\Rightarrow$ \texttt{MSG\_ERROR} ``You already submitted an action this round.''\ (server.c:322--324); original action preserved. (3)~Invalid \texttt{action\_id} or dead/out-of-range target $\Rightarrow$ \texttt{MSG\_ERROR} with specific string from \texttt{game\_validate\_action} (server.c:331--334); connection stays open so player can resubmit. \textbf{Client send:} \texttt{write\_all} failure $\Rightarrow$ \texttt{perror("write")} + \texttt{exit(1)} (client.c:449--452). Client also guards locally: a targeted move submitted without a target ID prints a usage hint without sending (client.c:436--440). \\[4pt]
...
\textbf{Server:} (1)~Non-host sender $\Rightarrow$ \texttt{MSG\_ERROR} ``Only the host can start.''\ (server.c:358--361); connection left open. (2)~Game already started $\Rightarrow$ \texttt{MSG\_ERROR} ``Game already started.''\ (server.c:362--365). (3)~Any connected player has no character $\Rightarrow$ \texttt{MSG\_ERROR} ``All players must select a character first.''\ (server.c:371--377). (4)~Fewer than 2 players $\Rightarrow$ \texttt{MSG\_ERROR} ``Need at least 2 players to start.''\ (server.c:379--383). In all four cases the game stays in \texttt{STATE\_LOBBY} and no state is mutated. \textbf{Client send:} \texttt{write\_all} failure $\Rightarrow$ \texttt{perror("write")} + \texttt{exit(1)} (client.c:384--387). \\[4pt]
...
Sent during \texttt{STATE\_LOBBY} to claim character slot 0--7. On success, \texttt{MSG\_LOBBY\_UPDATE} and updated \texttt{MSG\_CHAR\_LIST} are broadcast to all players (server.c:303--307). &
\textbf{Server:} (1)~Not in lobby phase $\Rightarrow$ \texttt{MSG\_ERROR} ``Can't change character now.''\ (server.c:289--292); connection left open. (2)~\texttt{char\_id} out of range or character already taken $\Rightarrow$ \texttt{MSG\_ERROR} ``Character unavailable or invalid.''\ (server.c:294--297); player may retry with a different ID. \textbf{Client send:} \texttt{write\_all} failure $\Rightarrow$ \texttt{perror("write")} + \texttt{exit(1)} (client.c:369--372). \\
\bottomrule
\end{longtable}
\endgroup
...
\begingroup
\small
\begin{longtable}{@{}p{2.0cm} p{1.2cm} p{2.4cm} p{3.0cm} p{4.8cm}@{}}
\toprule
\textbf{Message / Opcode} & \textbf{Size} & \textbf{Encoding} & \textbf{Semantics \& Invariants} & \textbf{Error Handling} \\
\midrule\endhead
...
\textbf{Server write:} sent via \texttt{write\_all} in \texttt{send\_join\_ack} (server.c:206--214); failure is not checked individually. The subsequent \texttt{send\_lobby\_update} call uses \texttt{broadcast}, which checks \texttt{write\_all} for every slot and calls \texttt{disconnect\_player} on any failure (server.c:62--63), so the broken socket is detected within the same connection-handling step. \textbf{Client receive:} always structurally valid when sent; unknown opcodes from the server cause \texttt{process\_server\_buffer} to return \texttt{-1}, breaking the event loop (client.c:297--300). \\[4pt]
...
\textbf{Server write:} sent via \texttt{write\_all} in \texttt{send\_char\_list} (server.c:103); failure not individually checked. The \texttt{send\_lobby\_update} broadcast that immediately follows detects any broken socket and calls \texttt{disconnect\_player} (server.c:62--63). \textbf{Client receive:} always structurally valid. \\[4pt]
...
\textbf{Server write:} sent per-player via \texttt{write\_all} (server.c:123); failure not checked individually. Any player whose socket broke will be caught by the \texttt{send\_round\_start} broadcast (which calls \texttt{disconnect\_player}) that executes immediately after in \texttt{begin\_round}. \textbf{Client receive:} always structurally valid; stores \texttt{my\_id} and the action list for the remainder of the session. \\[4pt]
...
\texttt{uint8\_t padding[2];} &
Broadcast once per second during action collection (server.c, lines~609--626). When \texttt{seconds\_left} reaches 0 the server proceeds to resolution regardless of missing submissions. &
...
\textbf{Server write:} sent via \texttt{write\_all} only to players with \texttt{alive \&\& fd != -1} (server.c:481--487); write failure not individually checked. The \texttt{send\_round\_start} broadcast immediately preceding this loop has already disconnected any players with broken sockets (setting their \texttt{fd = -1}), so the alive-and-connected guard ensures no write is attempted to a dead socket. \textbf{Client receive:} always structurally valid. \\[4pt]
...
\textbf{Server write:} sent via \texttt{broadcast} (server.c:186); \texttt{write\_all} failure $\Rightarrow$ \texttt{disconnect\_player}. After this message the server enters \texttt{STATE\_GAME\_OVER} and shuts down once all player sockets close (server.c:710--718). \textbf{Client receive:} sets \texttt{game\_over = 1} and displays the result; the event loop continues until the socket closes, at which point \texttt{read} returns 0 and the client exits cleanly (client.c:529--531). \\[4pt]

\textbf{MSG\_WAIT}\\(0x18) &
4 bytes &
\texttt{uint8\_t msg\_type;}\newline
\texttt{uint8\_t padding[3];} &
Unicast acknowledgement sent to a player after their \texttt{MSG\_ACTION} is accepted (server.c:347). Signals the client to stop prompting for input and wait for resolution. &
\textbf{Server write:} sent via \texttt{write\_all} in \texttt{send\_wait} (server.c:198--204); failure not checked. Crucially, the player's \texttt{pending\_action\_id} is recorded \emph{before} the send (server.c:344--345), so the action is preserved even if delivery fails. The next \texttt{broadcast} will detect the broken socket. \textbf{Client receive:} sets \texttt{waiting\_for\_result = 1} and suppresses further input prompts; always structurally valid. \\[4pt]
...
\textbf{1. Client disconnection mid-game.} Every \texttt{read} on a player socket is checked for $n \leq 0$ (server.c, line~694). Zero means a clean peer close; negative means an I/O error. In both cases \texttt{disconnect\_player} is called (server.c, line~695), which closes the socket, sets \texttt{fd = -1} to remove it from the \texttt{fd\_set} on the next iteration, and frees the character slot. If the game is in progress, \texttt{game\_remove\_player} marks the player as eliminated and \texttt{MSG\_PLAYER\_ELIM} is broadcast to remaining players. \texttt{game\_check\_end} handles the edge case where a disconnect reduces survivors to one.

\textbf{2. Unknown or malformed opcode.} In \texttt{process\_player\_buffer} (server.c, lines~400--413), the \texttt{default} branch logs the unknown opcode and immediately calls \texttt{disconnect\_player}, preventing the server from interpreting arbitrary bytes as valid messages and corrupting game state. Symmetrically, in \texttt{process\_server\_buffer} (client.c, lines~297--300), an unknown server opcode causes a return of \texttt{-1}, which breaks the client event loop and closes the socket.

\textbf{3. Write failures (broken pipe).} \texttt{write\_all} returns \texttt{-1} on any \texttt{write(2)} failure including \texttt{EPIPE}. Helpers that use \texttt{broadcast} (server.c, lines~58--66) check the return value per player and call \texttt{disconnect\_player} on failure. Unicast helpers (\texttt{send\_error}, \texttt{send\_wait}, \texttt{send\_join\_ack}, \texttt{send\_char\_list}, \texttt{send\_game\_start}, \texttt{send\_status\_update}) do not check individually; broken sockets in those paths are caught at the next \texttt{broadcast} call. Because \texttt{disconnect\_player} sets \texttt{fd = -1} before any recursive broadcast it may trigger, no slot is written to twice after disconnection.

\textbf{4. \texttt{select} interrupted by signal (EINTR).} Both the server (server.c, line~607) and the client (client.c, line~520) check \texttt{errno == EINTR} after a negative \texttt{select} return and restart the loop with a \texttt{continue}. Any other negative return is treated as fatal and reported with \texttt{perror}:
\begin{lstlisting}
if (ready < 0) {
