...
\textbf{What can go wrong:} A client disconnects between a \texttt{select} poll and a subsequent \texttt{write}, causing \texttt{write(2)} to return $-1$ with \texttt{SIGPIPE} or \texttt{EPIPE}.

\textbf{How it is handled:} \texttt{write\_all} (server.c, lines~28--39) returns $-1$ if any call to \texttt{write} fails. Currently, the broadcast path does not check the return value of \texttt{write\_all} for every client; this is an acknowledged limitation. For unicast messages (errors, wait, join ack) the calling context guards the write with a socket validity check. The OS will deliver \texttt{SIGPIPE} on a closed socket write; in a production-grade server this would be suppressed with \texttt{signal(SIGPIPE, SIG\_IGN)}. Failed individual player writes result in the server logging the disconnect on the next readable event.
